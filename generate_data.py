...
print("🚢 Génération de données réalistes pour port sec...")
...
for i in range(1, 51):
    clients_data.append({
        ...
        'date_premier_contrat': datetime(2023, 1, 1) + timedelta(days=random.randint(0, 365)),
...
# 3. Opérations journalières (le cœur du système)
print("⏳ Génération des opérations (cela peut prendre 30 secondes)...")
...
while current_date <= date_fin:
    ...
    for _ in range(nb_operations):
        ...
        operations_data.append({
            ...
            'timestamp': current_date + timedelta(hours=heure),
...
print(f"✅ Opérations générées: {len(operations_df):,}")
...
print(f"✅ Données GPS générées: {len(gps_df)}")
...
print(f"✅ Données météo générées: {len(meteo_df)}")
